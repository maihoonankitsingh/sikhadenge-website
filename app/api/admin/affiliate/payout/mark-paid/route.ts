...
export async function POST(req: NextRequest) {
  try {
    ...
    const payoutReference = (body.payoutReference || "").trim() || null;
    ...
    const row = await prisma.affiliatePayout.findUnique({
      where: { id: payoutId },
      select: { id: true, status: true, payoutReference: true },
    });
    ...
    if (row.status === AffiliatePayoutStatus.PAID) {
      return NextResponse.json(
        { ok: false, error: "Payout is already marked paid." },
    ...
    if (row.status === AffiliatePayoutStatus.CANCELLED) {
      return NextResponse.json(
        { ok: false, error: "Cancelled payout cannot be marked paid." },
        withAffiliateCors({ status: 409 })
      );
    }
    ...
    const updated = await prisma.affiliatePayout.update({
      ...
      data: {
        ...
        payoutReference: payoutReference || row.payoutReference,
      },
      select: {
        id: true,
        status: true,
    ...
    return NextResponse.json(
      {
        ok: true,
        message: "Payout marked paid successfully.",
        payout: updated,
      },
      withAffiliateCors({ status: 200 })
    );
  } catch (error) {
    console.error("Affiliate payout mark-paid error:", error);
    return NextResponse.json(
