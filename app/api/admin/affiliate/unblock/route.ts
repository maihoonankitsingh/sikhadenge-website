...
export async function POST(req: NextRequest) {
  try {
    const body = (await req.json()) as UnblockPayload;
    ...
    if (partner.status !== AffiliateStatus.BLOCKED) {
      return NextResponse.json(
        { ok: false, error: "Only blocked affiliate can be unblocked." },
    ...
    return NextResponse.json(
      {
        ok: true,
        message: "Affiliate unblocked successfully.",
        affiliate: updated,
      },
      withAffiliateCors({ status: 200 })
    );
  } catch (error) {
    console.error("Affiliate unblock error:", error);
    return NextResponse.json(
