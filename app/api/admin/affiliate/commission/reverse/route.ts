export const dynamic = "force-dynamic";
export const revalidate = 0;

import { NextRequest, NextResponse } from "next/server";
import { affiliateCorsPreflight, withAffiliateCors } from "@/lib/api-cors";
import { PrismaClient, AffiliateCommissionStatus } from "@prisma/client";

const prisma = new PrismaClient();

export async function OPTIONS() {
  return affiliateCorsPreflight();
}

type ReversePayload = {
  commissionId?: string;
  reason?: string;
};

export async function POST(req: NextRequest) {
  try {
    const body = (await req.json()) as ReversePayload;
    const commissionId = (body.commissionId || "").trim();
    const reason = (body.reason || "").trim();

    if (!commissionId) {
      return NextResponse.json(
        { ok: false, error: "commissionId is required." },
        withAffiliateCors({ status: 400 })
      );
    }

    const row = await prisma.affiliateCommission.findUnique({
      where: { id: commissionId },
      select: {
        id: true,
        status: true,
        remarks: true,
      },
    });

    if (!row) {
      return NextResponse.json(
        { ok: false, error: "Commission not found." },
        withAffiliateCors({ status: 404 })
      );
    }

    if (row.status === AffiliateCommissionStatus.REVERSED) {
      return NextResponse.json(
        { ok: false, error: "Commission is already reversed." },
        withAffiliateCors({ status: 409 })
      );
    }

    const mergedRemarks = [
      row.remarks?.trim() || null,
      reason ? `[REVERSE_REASON] ${reason}` : null,
    ]
      .filter(Boolean)
      .join("\n\n");

    const updated = await prisma.affiliateCommission.update({
      where: { id: commissionId },
      data: {
        status: AffiliateCommissionStatus.REVERSED,
        reversedAt: new Date(),
        remarks: mergedRemarks || row.remarks,
      },
      select: {
        id: true,
        status: true,
        approvedAt: true,
        paidAt: true,
        reversedAt: true,
        commissionAmount: true,
        remarks: true,
        affiliatePartner: {
          select: {
            fullName: true,
            affiliateCode: true,
          },
        },
      },
    });

    return NextResponse.json(
      {
        ok: true,
        message: "Commission reversed successfully.",
        commission: updated,
      },
      withAffiliateCors({ status: 200 })
    );
  } catch (error) {
    console.error("Affiliate commission reverse error:", error);
    return NextResponse.json(
      { ok: false, error: "Internal server error." },
      withAffiliateCors({ status: 500 })
    );
  }
}
