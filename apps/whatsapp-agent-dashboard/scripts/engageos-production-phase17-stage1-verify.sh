...
transition_tuple="$(psql_scalar "SELECT COALESCE(\"fromStage\",'<null>') || '|' || \"toStage\" || '|' || COALESCE(\"fromMode\",'<null>') || '|' || \"toMode\" || '|' || COALESCE(\"fromWritePolicy\",'<null>') || '|' || \"toWritePolicy\" || '|' || COALESCE(CASE WHEN \"fromExternalWritesAllowed\" THEN 'true' ELSE 'false' END,'<null>') || '|' || CASE WHEN \"toExternalWritesAllowed\" THEN 'true' ELSE 'false' END || '|' || COALESCE(\"expectedVersion\"::text,'<null>') || '|' || \"resultingVersion\"::text FROM \"EngageControlledLaunchTransition\" WHERE \"workspaceId\" = '${WORKSPACE_ID}';")"
