...
for script in "$migrate" "$bootstrap" "$verify" "$batch"; do
  test -f "$script"
  bash -n "$script"
done
test -f "$bootstrap_ts"
...
# Orchestrator must always execute the dynamic migration after compatibility handling,
# and only bootstrap Stage1 after existing post-deploy + rollback-readiness gates.
