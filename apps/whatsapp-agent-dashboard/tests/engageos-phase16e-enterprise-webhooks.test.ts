...
class MemoryRepository
  implements EnterpriseWebhookRepository {
  ...
  async createEndpoint(
    input:
      Parameters<
        EnterpriseWebhookRepository["createEndpoint"]
      >[0],
    audit:
      WebhookSecurityAuditWrite,
  ) {
  ...
  async setEndpointStatus(
    input:
      Parameters<
        EnterpriseWebhookRepository["setEndpointStatus"]
      >[0],
    audit:
      WebhookSecurityAuditWrite,
  ) {
...
async function main() {
  ...
  const now =
    new Date(
      "2026-09-12T14:15:00.000Z",
    );

  const created =
    await createEnterpriseWebhookEndpoint(
      repository,
      {
        workspaceId:
          "workspace-a",
        ...
        name:
          " Enrollment Events ",
        url:
          "https://Hooks.Example.com/engage?source=sikhadenge",
        events: [
          "lead.enrolled",
          "lead.enrolled",
          "Payment.Succeeded",
  ...
  assert.equal(
    created.endpoint.status,
    "ACTIVE",
  );
  assert.deepEqual(
    created.endpoint.events,
    [
      "lead.enrolled",
      "payment.succeeded",
    ],
  ...
  assert.ok(
    created.signingSecret.length >= 32,
  );
  assert.ok(
    !("encryptedSecret" in created.endpoint),
  );
  ...
  assert.ok(stored);
  assert.notEqual(
    stored.encryptedSecret.ciphertext,
  ...
  const safeList =
    await listEnterpriseWebhookEndpoints(
      repository,
      "workspace-a",
    );
  ...
  assert.ok(
    !("encryptedSecret" in safeList[0]),
  );
  assert.ok(
    !JSON.stringify(safeList).includes(
      created.signingSecret,
    ),
  );
  ...
  const prepared =
    await prepareEnterpriseWebhookDelivery(
      repository,
      {
        workspaceId:
          "workspace-a",
  ...
  assert.equal(
    await verifyEnterpriseWebhookDelivery(
      repository,
      {
        workspaceId:
          "workspace-a",
  ...
  await pauseEnterpriseWebhookEndpoint(
    repository,
    {
      workspaceId:
        "workspace-a",
  ...
  await revokeEnterpriseWebhookEndpoint(
    repository,
    {
      workspaceId:
        "workspace-a",
  ...
  console.log(
    "Phase 16E enterprise webhook controls passed.",
  );
}
