...
function buildBetterTitleFromSlug(slug) {
  ...
  const captureFamilyAudience = (prefix) => {
    ...
    return {
      family: toTitleCase(match[1]),
      audience: toTitleCase(match[2]),
      year: match[3],
    };
  };
  ...
  if (stepGuide) {
    return `${stepGuide.family} for ${stepGuide.audience}: Step-by-Step Guide (${stepGuide.year})`;
  }
...
function trimTitle(title) {
  ...
  return title
    ...
    .replace(/\s+for Working Professionals/i, "")
    .replace(/\s+for Small Business Owners/i, "")
    .replace(/\s+for Content Creators/i, "")
    .replace(/\s+for College Students/i, "")
    .replace(/\s+for Instagram Creators/i, "")
    .replace(/\s+/g, " ")
    .trim()
    .slice(0, 68)
    .replace(/[\s:-]+$/, "");
