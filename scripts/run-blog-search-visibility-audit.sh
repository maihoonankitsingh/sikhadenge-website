...
PORT="${AUDIT_PORT:-3111}"
OUT="${AUDIT_OUT:-/root/sikhadenge-blog-search-visibility-$(date +%Y%m%d_%H%M%S)}"
BRANCH="${AUDIT_BRANCH:-redesign/blog-template-20260725}"

mkdir -p "$OUT"

if [ -L "$STAGE/node_modules" ]; then
  TARGET="$(readlink -f "$STAGE/node_modules" 2>/dev/null || true)"
  if [ "$TARGET" = "$ROOT/node_modules" ]; then
    rm -f "$STAGE/node_modules"
  else
    echo "Unexpected node_modules symlink: ${TARGET:-unresolved}" >&2
    exit 1
  fi
...
test -f "$STAGE/.next/prerender-manifest.json"
...
if [ -z "$BROWSER" ]; then
  ...
  if [ -n "$PLAYWRIGHT_BROWSER" ] && [ -x "$PLAYWRIGHT_BROWSER" ]; then
...
(
  ...
  node scripts/audit-blog-search-visibility.mjs
...
echo
echo "===== BLOG SEARCH VISIBILITY FINAL ====="
...
echo
cat "$OUT/search-visibility-audit.txt" 2>/dev/null || true
