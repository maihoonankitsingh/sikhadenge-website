...
check("local Kling asset", page.includes('/ai-video-kling-mark.svg'));
check("local Higgsfield asset", page.includes('/ai-video-higgsfield-mark.svg'));
check("canonical metadata", page.includes('rel="canonical"') && page.includes(canonicalUrl));
check("page-specific Open Graph", page.includes('property="og:title"') && page.includes('property="og:url"'));
...
check("keyboard tab navigation", page.includes('event.key === "ArrowLeft"') && page.includes('event.key === "ArrowRight"'));
...
check("below-fold render containment", css.includes('content-visibility: auto'));
...
console.log("\nScore:", `${checks.filter((item) => item.ok).length}/${checks.length}`);
