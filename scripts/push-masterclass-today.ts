...
async function main() {
  const start = new Date("2026-03-21T00:00:00.000+05:30");
  const end   = new Date("2026-03-22T00:00:00.000+05:30");
  ...
  for (const row of rows) {
    ...
    try {
      ...
      ok++;
      console.log(`PUSH_OK id=${row.id} phone=${phone} createdAt=${row.createdAt.toISOString()} resp=${String(out).slice(0,200)}`);
    } catch (err) {
      fail++;
      console.error(`PUSH_FAIL id=${row.id} phone=${phone} createdAt=${row.createdAt.toISOString()} err=${err instanceof Error ? err.message : String(err)}`);
    }
  }
