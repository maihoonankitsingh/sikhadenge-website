...
async function main() {
  const start = new Date("2026-03-18T00:00:00.000+05:30");
  const end = new Date();
  ...
  for (const row of rows) {
    ...
    try {
      ...
      ok++;
      console.log(
        `PUSH_OK id=${row.id} phone=${phone} createdAt=${row.createdAt.toISOString()} resp=${String(out).slice(0, 300)}`
      );
    } catch (err) {
      fail++;
      console.error(
