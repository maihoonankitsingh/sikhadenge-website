...
TEMP_OUT = ROOT / "data/tmp/generated-seo-tools-batch2-temp.json"
REPORT = ROOT / "reports/tool_families_batch2_temp_report.txt"
TOOLS = json.loads((ROOT / "config/seo/tool-family-heads-batch2.json").read_text())["tools"]
...
global_patterns = [
    ...
    ("expert-in-{tool}", "Expert in {label}"),
    ("master-in-{tool}", "Master in {label}"),
...
for tool in TOOLS:
    ...
    for suffix, title_tpl in global_patterns:
        if suffix == "":
            ...
            slug = suffix.format(tool=tool)
            if not slug.startswith(f"{tool}-") and slug != tool:
                slug = f"{tool}-{slug}"
        title = title_tpl.format(label=label, tool=tool)
        if slug not in existing:
...
lines = ["=== TOOL FAMILIES BATCH 2 TEMP REPORT ==="]
for tool in TOOLS:
