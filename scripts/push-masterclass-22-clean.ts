...
async function main() {
  ...
  const finalRows = Array.from(phoneMap.values()).sort((a, b) => a.createdAt.localeCompare(b.createdAt));
  ...
  console.log(`DONE ok=${ok} fail=${fail} skip=${skip} total=${finalRows.length}`);
}
