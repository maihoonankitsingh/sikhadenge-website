...
TEMP_OUT = ROOT / "data/tmp/generated-seo-pending30-temp-batch3.json"
REPORT = ROOT / "reports/pending30_temp_batch3_report.txt"
...
city_patterns = [
    ...
    ("best-tools-in-{city}", "Best {label} Tools in {City}")
]
...
audience_patterns = [
    ...
    ("roadmap-2026-for-{aud}", "{label} Roadmap 2026 for {Aud}")
]
...
question_patterns = [
    ...
    ("for-college-students", "{label} for College Students"),
    ("for-school-students", "{label} for School Students"),
...
for family in pending:
    ...
    for city in cities:
        ...
        for suffix_tpl, title_tpl in city_patterns:
            ...
            if slug not in existing:
                item = make_page(family, slug, title, city=city, kind="city_intent_batch3", modifier="local batch3")
    ...
    for aud in audiences:
        ...
        for suffix_tpl, title_tpl in audience_patterns:
            ...
            if slug not in existing:
                item = make_page(family, slug, title, audience=aud, kind="audience_intent_batch3", modifier="audience batch3")
                new_items.append(item)
                existing.add(slug)
                per_family[family] += 1
                if len(samples[family]) < 5:
    ...
    for suffix, title_tpl in question_patterns:
        ...
        if slug not in existing:
            item = make_page(family, slug, title, kind="question_intent_batch3", modifier=suffix.replace("-", " "))
            new_items.append(item)
            existing.add(slug)
            per_family[family] += 1
            if len(samples[family]) < 5:
