      ...
      <main className={`workshop-shell workshop-theme-${config.theme}`}>
        ...
        <section className="workshop-hero">
          <div className="workshop-hero-copy">
            ...
            <p>
              The live masterclass shows what is possible. This guided workshop is where you implement the workflows yourself, complete practical exercises and leave with reusable AI systems.
            </p>
            <div className="workshop-proof-row">
              <span>Live guided sessions</span>
              <span>Daily implementation</span>
              <span>Assignments + project</span>
          ...
          <aside className="workshop-offer-card">
            ...
            {config.workshopRegularPrice > config.workshopPrice ? (
              <div className="workshop-price-anchor">Regular ₹{config.workshopRegularPrice}</div>
            ) : null}
            ...
            <button type="button" onClick={startCheckout} disabled={status === "loading"} className="workshop-primary-button">
              {status === "loading" ? "Opening secure checkout…" : `Join Workshop for ₹${config.workshopPrice}`}
            </button>
        ...
        <section className="workshop-section workshop-section-light">
          <div className="workshop-section-head">
            <span className="workshop-kicker">THE GAP THIS WORKSHOP CLOSES</span>
            <h2>Watching AI demos is not the same as building an AI workflow you can use tomorrow.</h2>
          </div>
          <div className="workshop-three-grid">
            <article><strong>01</strong><h3>From random prompts</h3><p>Move toward reusable instructions and repeatable output structures.</p></article>
            <article><strong>02</strong><h3>From tool curiosity</h3><p>Build practical workflows around real tasks rather than collecting more AI tools.</p></article>
            <article><strong>03</strong><h3>From watching to doing</h3><p>Complete guided exercises and a mini-project so the learning becomes executable.</p></article>
          </div>
        </section>

        <section className="workshop-section">
          <div className="workshop-section-head">
            <span className="workshop-kicker">IMPLEMENTATION ROADMAP</span>
            <h2>Your {config.productLabel} workshop journey.</h2>
            <p>The exact live schedule can be compressed to 5 days or expanded to 7 without changing the core implementation outcomes.</p>
          </div>
          <div className="workshop-curriculum">
            {curriculum.map(([day, title]) => (
              <article key={day}><span>{day}</span><strong>{title}</strong></article>
            ))}
          </div>
        </section>
        ...
        <section className="workshop-section workshop-section-light">
          <div className="workshop-section-head">
            <span className="workshop-kicker">YOU LEAVE WITH OUTPUTS</span>
            <h2>Not just notes. Working assets and repeatable systems.</h2>
          </div>
          <div className="workshop-deliverables">
            {sharedDeliverables.map((item) => <div key={item}><span>✓</span><p>{item}</p></div>)}
          </div>
        </section>

        <section className="workshop-section workshop-mentor-grid">
          <div className="workshop-mentor-image">
            <Image src="/funnels/shared/mentor.jpg" width={680} height={680} alt="SikhaDenge workshop mentor" />
          </div>
          <div>
            <span className="workshop-kicker">GUIDED BY SIKHADENGE</span>
            <h2>Implementation first. Tool hype second.</h2>
            <p>
              The workshop is designed around practical digital work, structured exercises and reviewable outputs. Sikhadenge remains the education provider; {config.productLabel} is used as the product being taught, not as an implied brand partnership.
            </p>
            <ul>
              <li>Practical live walkthroughs</li>
              <li>Guided learner implementation</li>
              <li>Frameworks you can reuse after the batch</li>
              <li>Clear bridge into advanced learning only if you want it</li>
            </ul>
          </div>
        </section>

        <section className="workshop-section workshop-section-light">
          <div className="workshop-section-head"><span className="workshop-kicker">FAQ</span><h2>Before you enroll.</h2></div>
          <div className="workshop-faqs">
            {faqs.map(([q, a]) => <details key={q}><summary>{q}</summary><p>{a}</p></details>)}
          </div>
        </section>

        <section className="workshop-final-cta">
          <span className="workshop-kicker">YOUR NEXT IMPLEMENTATION STEP</span>
          <h2>Turn the masterclass insight into a system you can actually use.</h2>
          <p>{config.workshopName} • current configured cohort price ₹{config.workshopPrice}</p>
          <button type="button" onClick={startCheckout} disabled={status === "loading"} className="workshop-primary-button">
            {status === "loading" ? "Opening secure checkout…" : `Join for ₹${config.workshopPrice}`}
          </button>
          {!leadId ? <small>Open this page from the personalized link sent to your registered WhatsApp number to continue.</small> : null}
        </section>

        <footer className="workshop-footer">
          <span>SikhaDenge • ThinkGrow Private Limited</span>
          <p>SikhaDenge is an independent education provider. ChatGPT is a product of OpenAI and Claude is a product of Anthropic. No affiliation or endorsement is implied.</p>
