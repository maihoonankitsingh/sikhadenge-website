...
OUT="$ARTIFACT_ROOT/blog-review-preview-production-switch-preflight-v1-$TS"
...
PM2_COUNT="$(jq --arg name "$PM2_NAME" '[.[] | select(.name==$name)] | length' "$PM2_RAW")"
test "$PM2_COUNT" = "1" || fail "pm2_process_count_invalid"
...
TOKEN_LENGTH="$(awk -F= '/^[[:space:]]*BLOG_REVIEW_PREVIEW_TOKEN=/{sub(/^[^=]*=/,""); gsub(/^["'"']|["'"']$/,""); print length($0)}' "$ROOT/.env.local" | tail -n1)"
