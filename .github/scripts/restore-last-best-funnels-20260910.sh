...
recover_asset() {
  ...
  if grep -Fq "location = $path {" "$CLAUDE_ASSETS"; then return 0; fi
...
while IFS= read -r u; do
  ...
  case "$code" in
    200|204|301|302|304) ;;
    *) echo "MISSING_CLAUDE_ASSET http=$code url=$u"; recover_asset "$u"; recovered=1 ;;
  esac
...
grep -iE '^(HTTP/|x-sd-claude-snapshot:|content-type:|content-length:)' "$BK/claude.after.headers" || true
...
t = html.unescape(re.sub(r'\s+', ' ', re.sub(r'<[^>]+>', ' ', t))).strip().lower()
checks = ['master 25+ ai tools', 'ai tools', 'better results']
print('CLAUDE_TEXT_MARKERS=' + ','.join(f'{c}:{c in t}' for c in checks))
if not ('master 25+ ai tools' in t or ('ai tools' in t and 'better results' in t)):
    raise SystemExit('Claude visible-text fingerprint failed')
PY
