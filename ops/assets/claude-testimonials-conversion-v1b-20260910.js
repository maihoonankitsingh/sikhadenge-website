(() => {
  ...
  if (window.__SD_CLAUDE_TESTIMONIALS_CONVERSION_V1B__) return;
  window.__SD_CLAUDE_TESTIMONIALS_CONVERSION_V1B__ = true;

  const ROOT_ID = 'sd-claude-testimonials-v305';
  const STYLE_ID = 'sd-claude-testimonials-conversion-v1b-style';
  ...
  function applyResponsiveMode(root) {
    ...
    const cards = Array.from(firstSet?.querySelectorAll('.sd-v305-card') || []);

    if (mobileQuery.matches) {
      setAttr(root, 'data-sd-testimonial-mobile-mode', 'manual-swipe');
      setImportant(viewport, 'overflow-x', 'auto');
      setImportant(viewport, 'overflow-y', 'hidden');
      setImportant(viewport, 'scroll-snap-type', 'x mandatory');
      setImportant(viewport, 'scroll-padding-inline', '16px');
      setImportant(viewport, 'overscroll-behavior-x', 'contain');
      setImportant(viewport, 'scrollbar-width', 'none');
      setImportant(track, 'animation', 'none');
      setImportant(track, 'transform', 'none');
      setImportant(track, 'will-change', 'auto');
      setImportant(firstSet, 'padding-left', '16px');
      setImportant(secondSet, 'display', 'none');
      cards.forEach(card => {
    ...
    setAttr(root, 'data-sd-testimonial-mobile-mode', 'marquee');
    clearInline(viewport, ['overflow-x','overflow-y','scroll-snap-type','scroll-padding-inline','overscroll-behavior-x','scrollbar-width']);
    clearInline(track, ['animation','transform','will-change']);
    clearInline(firstSet, ['padding-left']);
    clearInline(secondSet, ['display']);
    cards.forEach(card => clearInline(card, ['scroll-snap-align','scroll-snap-stop']));
  }
  ...
  function apply() {
    ...
    setAttr(root, 'data-sd-testimonials-conversion', 'v1b');
