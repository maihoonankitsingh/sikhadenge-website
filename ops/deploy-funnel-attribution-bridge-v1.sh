...
set -Eeuo pipefail
TS="$(date +%Y%m%d-%H%M%S)"
MAIN=/etc/nginx/sites-enabled/sikhadenge.in-ssl
...
mkdir -p "$BACK" "$DIR"
cp -a "$MAIN" "$BACK/sikhadenge.in-ssl.before"
cp -a "$REG" "$BACK/registration-hot.conf.before"
...
import sys
main=Path(sys.argv[1]); reg=Path(sys.argv[2])
...
def inject(path, headers):
    s=path.read_text()
    for header in headers:
        a,op,b=bounds(s,header)
        block=s[a:b]
        if marker not in block:
            s=s[:op+1]+directive+s[op+1:]
    path.write_text(s)

s=main.read_text()
if 'location = /funnel-attribution-bridge-v1.js {' not in s:
    pos=s.find('location = /masterclass/ai-video {')
    if pos < 0: raise SystemExit('AI Video location missing for static asset insertion')
    s=s[:pos]+static+s[pos:]
    main.write_text(s)
inject(main,['location = /masterclass/ai-video {','location = /masterclass/claude/free {'])
inject(reg,['location = /gen-ai-masterclass/register-one-step {'])
PY

rollback(){
  echo 'ROLLBACK_FUNNEL_ATTRIBUTION_BRIDGE'
  cp -a "$BACK/sikhadenge.in-ssl.before" "$MAIN"
  cp -a "$BACK/registration-hot.conf.before" "$REG"
