  ...
  for (const [name, width, height, mobile] of [
    ...
    const landingState = await page.evaluate(() => {
      ...
      return {
        ...
        ctaPath: cta ? new URL(cta.href, location.href).pathname : '',
        ctaHref: cta?.href || '',
    ...
    if (
      landingState.sections !== 18 ||
      !landingState.h1.includes('Master Claude + 25+ AI Tools') ||
      !landingState.finalText.includes('Ready to build your first practical AI workflow?') ||
      landingState.ctaPath !== '/gen-ai-masterclass/register-one-step' ||
      landingState.overflow
    ) {
      throw new Error(`${name} landing regression: ${JSON.stringify(landingState)}`);
    }
    ...
    const registrationState = await page.evaluate(() => {
      ...
      const nativeMain = [...document.querySelectorAll('main')].find((m) => {
        if (m.closest('#sdv2-root') || m.querySelector('#sdv2-root')) return false;
        const text = n(m.textContent);
        return text.includes('Book free masterclass') && text.includes('Direct registration on the same page.');
      });
      ...
      return {
        ...
        params,
        scriptCount,
        htmlFlag: document.documentElement.getAttribute('data-sd-registration-native-suppress-v1') || '',
        rootFlag: root?.getAttribute('data-sd-registration-native-suppress') || '',
        rootVisible: visible(root),
        rootPosition: root ? getComputedStyle(root).position : '',
        rootZ: root ? Number(getComputedStyle(root).zIndex) : 0,
        nativeFound: !!nativeMain,
        nativeDisplay: nativeMain ? getComputedStyle(nativeMain).display : '',
        nativeAriaHidden: nativeMain?.getAttribute('aria-hidden') || '',
        nativeInert: nativeMain ? nativeMain.inert === true || nativeMain.hasAttribute('inert') : false,
        nativeMarker: nativeMain?.getAttribute('data-sd-registration-native-suppressed') || '',
        nativeVisible: visible(nativeMain),
        oldSubmitVisible: visible(oldSubmit),
        newContinueVisible: visible(newContinue),
        bodyHasOldText: n(document.body.innerText).includes('Book free masterclass'),
        bodyHasV72Text: n(document.body.innerText).includes('Reserve your free seat'),
        overflow: document.documentElement.scrollWidth > document.documentElement.clientWidth + 2
      };
    });
    ...
    for (let i = 0; i < 12; i++) {
      ...
      keyboardFocus.push(await page.evaluate(() => {
        ...
        return {
          ...
          text: String(e?.textContent || e?.value || '').replace(/\s+/g, ' ').trim().slice(0, 120)
        };
      }));
    }

    const oldKeyboardHit = keyboardFocus.some((f) =>
      f.placeholder === 'Enter name' ||
      f.placeholder === 'Email' ||
      f.placeholder === 'Phone number' ||
      /Register for\s*₹1999/i.test(f.text)
    );
    const v72KeyboardHit = keyboardFocus.some((f) =>
      ['sd-reg-name', 'sd-reg-email', 'sd-reg-phone'].includes(f.id) ||
      /Continue\s*—\s*2 quick questions/i.test(f.text)
    );

    const structuralOK =
      registrationState.path === '/gen-ai-masterclass/register-one-step' &&
      registrationState.params.source === 'claude-masterclass' &&
      registrationState.params.utm_source === 'internal_qa' &&
      registrationState.params.utm_medium === 'e2e' &&
      registrationState.params.utm_campaign === campaign &&
      registrationState.params.utm_content === 'final_cta' &&
      registrationState.scriptCount === 1 &&
      registrationState.htmlFlag === '1' &&
      registrationState.rootFlag === 'v1' &&
      registrationState.rootVisible &&
      registrationState.rootPosition === 'fixed' &&
      registrationState.rootZ > 1000000 &&
      registrationState.nativeFound &&
      registrationState.nativeDisplay === 'none' &&
      registrationState.nativeAriaHidden === 'true' &&
      registrationState.nativeInert &&
      registrationState.nativeMarker === 'v1' &&
      !registrationState.nativeVisible &&
      !registrationState.oldSubmitVisible &&
      registrationState.newContinueVisible &&
      !registrationState.bodyHasOldText &&
      registrationState.bodyHasV72Text &&
      !registrationState.overflow &&
      !oldKeyboardHit &&
      v72KeyboardHit;
    ...
    if (!structuralOK || badAssets.length) {
      failures.push(`${name} suppression: ${JSON.stringify({ registrationState, keyboardFocus, badAssets })}`);
      await page.close();
      continue;
    }
    ...
    page.on('request', (req) => {
      const url = req.url();
      if (req.method() === 'POST' && url.includes('/api/masterclass/lead')) {
        intercepted.push({ url, postData: req.postData() || '' });
        return req.respond({
          status: 200,
          contentType: 'application/json',
          body: JSON.stringify({ id: `qa-intercepted-${name}`, success: true })
        });
      }
      if (req.isNavigationRequest() && /^https?:/.test(url) && !url.startsWith('https://sikhadenge.in/')) {
        return req.abort();
      }
      req.continue();
    });
    ...
    let payload = null;
    try { payload = intercepted[0] ? JSON.parse(intercepted[0].postData) : null; } catch (_) {}
    const confirmationState = await page.evaluate(() => {
      const n = (s) => String(s || '').replace(/\s+/g, ' ').trim();
      const root = document.getElementById('sdv2-root');
      const welcome = root?.querySelector('[data-action="welcome"]');
      return {
        text: n(root?.innerText).slice(0, 4500),
        welcomeVisible: !!welcome && getComputedStyle(welcome).display !== 'none' && welcome.getBoundingClientRect().width > 0,
        oldVisibleText: n(document.body.innerText).includes('Register for ₹1999'),
        overflow: document.documentElement.scrollWidth > document.documentElement.clientWidth + 2
      };
    });

    const payloadOK = !!payload &&
      payload.name === 'QA Section Twelve' &&
      payload.email === 'qa.section12@example.com' &&
      payload.phone === '9999999999' &&
      !!payload.experience &&
      !!payload.goal &&
      typeof payload.laptop === 'boolean' &&
      payload.utm_source === 'internal_qa' &&
      payload.utm_medium === 'e2e' &&
      payload.utm_campaign === campaign &&
      payload.utm_content === 'final_cta';
    ...
    if (!payloadOK || !confirmationOK) {
      failures.push(`${name} E2E: ${JSON.stringify({ payload, confirmationState, interceptedCount: intercepted.length })}`);
    }
