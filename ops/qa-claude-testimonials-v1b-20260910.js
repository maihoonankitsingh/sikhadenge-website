...
function errorSignature(errors) {
  ...
  for (const value of errors) {
    const match = String(value).match(/Minified React error #(\d+)/);
    const key = match ? `react-${match[1]}` : String(value);
...
(async () => {
  ...
  for (const [name, width, height, mobile] of [
    ...
    await page.goto(`https://sikhadenge.in/masterclass/claude/free?testimonials_v1b_qa=${name}-${Date.now()}`, {
    ...
    const state = await page.evaluate(() => {
      ...
      return {
        ...
        scriptPresent: [...document.scripts].filter(s => (s.src || '').includes('/claude-testimonials-conversion-v1b-20260910.js')).length,
        oldV1Present: [...document.scripts].some(s => /claude-testimonials-conversion-v1-20260910\.js(?:$|\?)/.test(s.src || '')),
        conversionFlag: root?.getAttribute('data-sd-testimonials-conversion') || '',
        countFlag: root?.getAttribute('data-sd-testimonial-count') || '',
        mobileMode: root?.getAttribute('data-sd-testimonial-mobile-mode') || '',
        ...
        runtimeFlag: !!window.__SD_CLAUDE_TESTIMONIALS_CONVERSION_V1B__,
      };
    });
    ...
    const baseTestimonialOK = state.conversionFlag === 'v1b' && state.countFlag === '6' && state.runtimeFlag && state.pill === pill && state.title === title && state.desc === desc && state.uniqueCards === 6 && state.totalCards === 12 && labelsOK && mediaOK;
    ...
    if (name === 'desktop') {
      responsiveOK = state.mobileMode === 'marquee' && Math.abs(state.cardSize[0] - 280) <= 1 && Math.abs(state.cardSize[1] - 420) <= 1 && state.animation[0] === 'sd-v305-marquee' && state.animation[1] === '36s' && state.secondDisplay !== 'none' && state.meta.find(x => x.text === 'Swipe to browse')?.display === 'none';
    } else if (name === 'tablet') {
      responsiveOK = state.mobileMode === 'marquee' && Math.abs(state.cardSize[0] - 258) <= 1 && Math.abs(state.cardSize[1] - 387) <= 1 && state.animation[0] === 'sd-v305-marquee' && state.animation[1] === '36s' && state.secondDisplay !== 'none' && state.meta.find(x => x.text === 'Swipe to browse')?.display === 'none';
    } else {
      manualScroll = await page.evaluate(async () => {
        const viewport = document.querySelector('#sd-claude-testimonials-v305 .sd-v305-viewport');
        if (!viewport) return { before: -1, after: -1 };
        const before = viewport.scrollLeft;
        viewport.scrollTo({ left: 320, behavior: 'auto' });
        await new Promise(resolve => setTimeout(resolve, 500));
        return { before, after: viewport.scrollLeft };
      });
      responsiveOK = state.mobileMode === 'manual-swipe' && Math.abs(state.cardSize[0] - 240) <= 1 && Math.abs(state.cardSize[1] - 360) <= 1 && state.animation[0] === 'none' && state.secondDisplay === 'none' && state.viewportScrollable && state.viewportOverflowX === 'auto' && state.viewportOverflowY === 'hidden' && state.scrollSnapType.includes('x') && state.firstCardSnap === 'start' && state.meta.find(x => x.text === 'Swipe to browse')?.display !== 'none' && manualScroll.after > manualScroll.before;
    }
    ...
    console.log(name, JSON.stringify({
      ...
      testimonial: {
        ...
        desc: state.desc,
        meta: state.meta,
        cards: state.uniqueCards,
        cardSize: state.cardSize,
        animation: state.animation,
        mobileMode: state.mobileMode,
        overflowX: state.viewportOverflowX,
        overflowY: state.viewportOverflowY,
        secondDisplay: state.secondDisplay,
        scrollSnapType: state.scrollSnapType,
      },
      manualScroll,
      ...
      oldV1Present: state.oldV1Present,
      interaction,
      legacyVisible,
      badAssets,
      errorCounts,
    ...
    if (
      ...
      state.scriptPresent !== 1 || state.oldV1Present || state.duplicateTabIndexes.some(value => value !== '-1') || badAssets.length || !errorsOK
    ) {
      failures.push(`${name}: state=${JSON.stringify(state)} manual=${JSON.stringify(manualScroll)} interaction=${JSON.stringify(interaction)} legacy=${legacyVisible.join('|')} bad=${badAssets.join('|')} errors=${JSON.stringify(errorCounts)}`);
    }
  ...
  console.log('CLAUDE_TESTIMONIALS_V1B_3VIEW_QA_PASS');
})().catch(error => {
  console.error(error);
  process.exit(1);
});
