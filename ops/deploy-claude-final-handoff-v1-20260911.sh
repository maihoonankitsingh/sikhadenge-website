...
NEW_NAME='free-802d96fd8696db68-v315r-20260830-204006-v316-20260830-204806-hero-v1-trust-v1-outcomes-v1-agenda-v1-audience-v1b-closing-v1-final-handoff-v1-20260911.js'
...
NEW_COPY='Reserve your free seat now. After registration, follow the confirmation and WhatsApp joining instructions to get ready for the live session — no payment required.'
...
if grep -Fq 'SIKHADENGE_CLAUDE_FINAL_HANDOFF_V1_20260911' "$SITE"; then echo 'Final handoff route marker already exists' >&2; exit 31; fi
if grep -Fq 'SIKHADENGE_CLAUDE_FINAL_HANDOFF_V1_ASSET_20260911' "$ASSETS"; then echo 'Final handoff asset marker already exists' >&2; exit 32; fi
...
print('FINAL_HANDOFF_CLIENT_CHUNK_PATCH=PASS')
PY
node --check "$NEW"
...
copy_filter='        sub_filter '+repr(oldcopy)+' '+repr(newcopy)+';\n'
css='<style id="sd-claude-free-final-handoff-v1">.claude-payment-trust-footer_paymentArea__aYkQy{display:none!important}.claude-payment-trust-footer_divider___nkaU{display:none!important}</style></head>'
css_filter='        sub_filter \'</head>\' '+repr(css)+';\n'
route=route.replace(new_line+'\n',new_line+'\n'+marker+copy_filter+css_filter,1)
...
code="$(curl -L -sS --retry 3 --retry-all-errors --connect-timeout 5 --max-time 35 -o "$BK/final-handoff.public.js" -w '%{http_code}' "https://sikhadenge.in${NEW_URL}")"
test "$code" = '200'
...
if newurl not in raw: raise SystemExit('new final handoff chunk URL missing from SSR')
if newcopy not in text: raise SystemExit('new final handoff paragraph missing from SSR text')
if oldcopy in text: raise SystemExit('old final handoff paragraph still visible in SSR')
if 'sd-claude-free-final-handoff-v1' not in raw: raise SystemExit('free-route payment-area hide style missing')
required=['Master Claude + 25+ AI Tools','FREE AI MASTERCLASS BONUS KIT','Learn AI. Apply it.','Frequently asked questions.','NEXT LIVE BATCH']
for x in required:
    if x not in text: raise SystemExit('SSR_MISSING '+repr(x))
for src in ['/claude-faq-conversion-v1-20260910.js?v=faq-conversion-v1-20260910','/claude-testimonials-conversion-v1b-20260910.js','/claude-bonus-value-v1-20260910.js','/funnel-attribution-bridge-v1.js?v=20260903-1']:
